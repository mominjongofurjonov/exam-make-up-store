...
const Products = () => {
  ...
  return (
    <div>
      <h1 style={{textAlign: 'center', marginTop: '50px', marginBottom: '40px'}}><i>All PRODUCTS</i></h1>
      {loading ? (
        <div className="loader"></div>
      ) : (
        <div className="all_cards_wrapper">
          {product.slice(0, 51).map((pr) => (
              <div key={pr.id} className="card_product" >
              <Link to={`/products/${pr.id}`}><img  className="card_images" width={200} src={pr.api_featured_image} alt="" /></Link>
              <br />
